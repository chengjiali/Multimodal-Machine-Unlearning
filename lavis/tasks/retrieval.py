...
class RetrievalTask(BaseTask):
    ...
    def evaluation(self, model, data_loader, **kwargs):
        ...
        if is_main_process():
            out = os.path.join(self.cfg.output_dir, f'image_embed_{split_name}.pt')
            torch.save(image_embed, out)

            out = os.path.join(self.cfg.output_dir, f'text_embed_{split_name}.pt')
            torch.save(text_embed, out)

            out = os.path.join(self.cfg.output_dir, f'score_i2t_{split_name}.pt')
            torch.save(score_i2t, out)

            out = os.path.join(self.cfg.output_dir, f'score_t2i_{split_name}.pt')
            torch.save(score_t2i, out)
