...
class MultimodalClassificationTask(BaseTask):
    ...
    def after_evaluation(self, val_result, split_name, epoch, **kwargs):
        ...
        torch.save(torch.vstack(val_result[3]), os.path.join(registry.get_path("result_dir"), f'hidden_state_{split_name}.pt'))

        if len(val_result[2]) > 0:
            torch.save(torch.vstack(val_result[1]), os.path.join(registry.get_path("result_dir"), f'image0_embed_{split_name}.pt'))
            torch.save(torch.vstack(val_result[2]), os.path.join(registry.get_path("result_dir"), f'image1_embed_{split_name}.pt'))
        else:
            torch.save(torch.vstack(val_result[1]), os.path.join(registry.get_path("result_dir"), f'image_embed_{split_name}.pt'))
